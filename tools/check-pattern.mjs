...
/* ---------- замечания ---------- */
{
  ...
  /* Прорыв стенки и запас прочности смотрят в разные стороны: вещь с глубокими
     ложбинами легче, и осевая модель даёт ей запас БОЛЬШЕ, чем гладкой. Человек
     поверит тому числу, которое зеленее, поэтому красное замечание обязано
     сказать, что на запас здесь смотреть нельзя. */
  const pierce = patternWarnings(sanitizePattern({layers: [{id: 'window', n: 10, depth: 5, m: 5}]}),
                                 {wall: 4, D: 160, H, bead: 4.2, hollow: true})
    .find(w => w.lvl === 'bad' && /прорв/.test(w.txt));
  if (!pierce) P('прорыв стенки не помечен красным');
  else if (!/запас прочности/i.test(pierce.txt))
    P('замечание о прорыве молчит про запас прочности, который при этом растёт');
  ...
  for (const layers of [[{id: 'flute', n: 12, depth: 1.5}], [{id: 'bump', n: 10, depth: 2, m: 5}],
    ...
    /* Общий список предваряет эти замечания словом «Узор: », поэтому сам текст
       с него начинаться не должен: «Узор: Узор и форма для отливки» человек
       читает как небрежность, и он прав. */
    for (const w of all)
      if (/^Узор[ :,]/i.test(w.txt))
        P(`замечание узора начинается со слова «Узор» — в списке оно удвоится: «${w.txt.slice(0, 34)}…»`);
    for (const w of all)
