...
/* Рельеф, растущий наружу, делает вещь шире гладкого профиля — а по этому
   числу решают, влезет ли она в печь и сколько станет на полку. Садка
   «как у гладкой» ставит соседей впритык. */
{
  const smooth = firedSize(prof, [], 0);
  /* Каннелюры, а не чешуя: у чешуи бугорки стоят рядами, и на высоте самой
     широкой точки этого профиля её вклад честно равен нулю — проверка мерила
     бы ноль и радовалась. Каннелюра идёт по всей высоте. */
  const bumped = firedSize(prof, [], 0, null, {pattern: {layers: [{id: 'flute', n: 12, depth: 3}]}});
  const grew = bumped.d - smooth.d;
  if (!(grew > 5.5 && grew < 6.5)) P(`рельеф 3 мм расширил садку на ${grew.toFixed(1)} мм вместо шести`);
  /* Рельеф внутрь габарит не уменьшает: касаются соседи гребнями, а гребень
     у лунок остаётся на гладкой стенке. */
  const inward = firedSize(prof, [], 0, null, {pattern: {layers: [{id: 'dimple', n: 12, depth: 3, m: 6}]}});
  if (inward.d < smooth.d - 1e-9) P(`лунки сузили габарит до ${inward.d.toFixed(1)} — соседи ставятся по гребню`);
  if (Math.abs(bumped.h - smooth.h) > 0.01) P('рельеф на стенке поднял высоту садки — этого он не делает');
  const off = firedSize(prof, [], 0, null, {pattern: {layers: [{id: 'flute', n: 12, depth: 3, mute: true}]}});
  if (Math.abs(off.d - smooth.d) > 1e-9) P('выключенный слой всё равно расширил садку');
  /* И обратно: без узора число обязано остаться прежним — иначе поправка
     тихо приписывается всем изделиям подряд. */
  if (Math.abs(firedSize(prof, [], 0, null, {}).d - smooth.d) > 1e-9) P('без узора габарит изменился');
}
