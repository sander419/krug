...
/* Пустое поле — это «не задано», а не ноль. `+null` и `+''` дают ноль,
   и поле, из которого стёрли число, молча превращало пояс слоя в пятипроцентную
   полоску у самого дна: человек стёр, чтобы напечатать другое, а рельеф уже уехал.
   Поэтому пустота и мусор одинаково означают умолчание. */
...
export function patternWarnings(pat, ctx = {}) {
  ...
  if (wall && carve > 0.01) {
    ...
    if (left < floor)
      out.push({lvl: 'bad', txt: `Рельеф срезает ${carve.toFixed(1)} мм при стенке ${wall} мм: ` +
        ...
        /* Число запаса прочности при этом может даже вырасти — вещь стала легче,
           а модель осевая и считает среднее сечение кольца. Молчать об этом
           нельзя: два числа смотрят в разные стороны, и человек поверит тому,
           которое зеленее. */
        ' Запас прочности считается по среднему сечению кольца и местного ' +
        'разрушения в ложбине не описывает — на него здесь не смотрите.'});
  ...
  if (ctx.look) {
    ...
    if (rc && rc.crest < 0.75)
      ...
      out.push({lvl: 'warn', area: 'glaze', help: 'glaze-run', txt:
        `глазурь в ложбинах наберётся ${rc.valley.toFixed(1)}× — там она темнее ` +
  ...
  out.push({lvl: 'warn', area: 'cast', help: 'casting', txt:
    'форма для отливки строится по гладкому профилю — рельеф ' +
