...
export function computeWarnings(state, prod, str){
  ...
  if(patternOn(pt)){
    ...
    if(state.firing==='glaze')
      /* `note` — оговорка о точности модели, а не дефект вещи. В «Контроле
         мастера» она стоит наравне с остальными (человеку важно её прочесть),
         но в причины готовности не идёт: «изделие не готово, потому что наша
         модель приблизительна» — это не про изделие. */
      w.push({lvl:'warn', area:'glaze', help:'glaze-thickness', note:true,
