...
export function sliceGCode(state){
  ...
  /* Крышку эта спираль тоже не печатает: она стоит на кромке, а спираль идёт
     от дна одним ходом. Её печатают вторым файлом — и в STL она отдельной
     деталью уже уезжает, значит сказать об этом обязаны здесь. */
  if(state.lid&&state.lid.on)
    warns.push({cls:'w',txt:'В G-code только корпус: крышка печатается отдельным заходом — '
      +'поставьте её на стол своим файлом (в STL она уходит отдельной деталью).'});
