...
/**
 * Ключ повода → id статьи. Ключей два рода, и оба законны: короткое имя
 * повода из карты выше («thinWall») и прямой id статьи («wall-thickness»).
 * Пока принимался только первый, пять кнопок «почему» не делали ничего —
 * и молча: обработчик не находил ключ и просто выходил.
 *
 * Разрешение живёт здесь, а не в интерфейсе, чтобы проверка и приложение
 * пользовались одной функцией: разрешай они порознь, проверка была бы
 * зелёной при мёртвой кнопке.
 */
export const helpArticleId = key =>
  (CONTEXT_HELP[key] && articleById(CONTEXT_HELP[key]) ? CONTEXT_HELP[key]
   : articleById(key) ? key : null);
