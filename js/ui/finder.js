...
const ACTIONS = [
  ...
  {name: 'Узор на стенке', ico: 'circle-dot', sel: '[data-block="pattern"] > .block-title',
   keys: 'рельеф слои каннелюры грани звезда кладка кора лунки ёлочка плетёнка чешуя спираль окна просвет литофания'},
  ...
  {name: 'Узор: Пояс (два слоя)', ico: 'circle-dot', run: () => patternPreset('belt'),
   keys: 'узор слои пояс чешуя кольцо лента полоса'},
  {name: 'Узор: Амфора (два слоя)', ico: 'circle-dot', run: () => patternPreset('amphora'),
   keys: 'узор слои каннелюры лунки ямки'},
  {name: 'Узор: Кора', ico: 'circle-dot', run: () => patternPreset('cortex'),
   keys: 'узор шум кора природный неровный'},
  {name: 'Узор: Кладка', ico: 'circle-dot', run: () => patternPreset('masonry'),
   keys: 'узор кирпич кладка ряды вразбежку'},
  {name: 'Узор: Ёлочка', ico: 'circle-dot', run: () => patternPreset('herringbone'),
   keys: 'узор зигзаг ёлочка шеврон'},
